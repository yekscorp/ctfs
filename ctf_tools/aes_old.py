...
def encrypt(inb, key, Nr=10, Nb=4):
    """
...
def decrypt(inb, key, Nr=10, Nb=4):
    """
    Return a tracker object that allows inspection of intermediate state
    of the AES encryption operation.
...
def encrypt_128(inb, key):
    if len(key) != 16:
        raise ValueError("Invalid key length for AES-128: %d" % (len(key),))
    return encrypt(inb, key, 10)

def encrypt_192(inb, key):
    if len(key) != 24:
        raise ValueError("Invalid key length for AES-192: %d" % (len(key),))
    return encrypt(inb, key, 12)

def encrypt_256(inb, key):
    if len(key) != 32:
        raise ValueError("Invalid key length for AES-256: %d" % (len(key),))
    return encrypt(inb, key, 14)

def encrypt_auto(inb, key):
    if len(key) == 16:
        return encrypt_128(inb, key)
    elif len(key) == 24:
        return encrypt_192(inb, key)
    elif len(key) == 32:
        return encrypt_256(inb, key)
